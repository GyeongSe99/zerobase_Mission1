...
<table class="tg" width="100%">
    <tr>
        ...
        <td class="tgHead">와이파이명</td>
        <td class="tgHead">등록일자</td>
        <td class="tgHead">비고</td>
    </tr>
    <%
        BookMarkDB bookMarkDB = new BookMarkDB();
        ArrayList<HashMap<String, String>> result = bookMarkDB.showWifiBookMarkList();

        if (result.size() > 0) {
            for (int i = 0; i < result.size(); i++) {
                HashMap data = result.get(i);
    %>
    <tr>
        <td class="tgBodyEven"><%=data.get("ID")%></td>
        <td class="tgBodyEven"><%=data.get("BOOKMARKNAME")%></td>
        <td class="tgBodyEven"><a href="../detail.jsp?wifiId=<%=data.get("wifi_ID")%>&LAT=<%=data.get("mylat")%>&LNT=<%=data.get("mylnt")%>"><%=data.get("WIFINAME")%></a></td>
        <td class="tgBodyEven"><%=data.get("wifi_register_date")%></td>
        <td class="tgBodyEven" align="center"><a href="deleteWifiBookmark.jsp?ID=<%=data.get("ID")%>">삭제</a></td>
    </tr>
    <%
        }
    } else {
